...
func placeOrder(productID string, quantity int, accountBalance float64) (bool, float64) {
	productStock := amountInStock(productID)
	if quantity > productStock {
		return false, accountBalance
	}

	oderCost := calcPrice(productID, quantity)
	if accountBalance < oderCost {
		return false, accountBalance
	}

	accountBalanceAfterOrder := accountBalance - oderCost
	return true, accountBalanceAfterOrder
